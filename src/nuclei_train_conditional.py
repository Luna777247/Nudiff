"""
Training script for Conditional Histopathology Image Synthesis
Stage 2 of NuDiff pipeline
"""
...
def visualize_conditional_samples(images, structures, save_path):
    """Visualize generated image samples with their conditions"""
    ...
    for i in range(n_samples):
        ...
        # Structure channels
        struct = structures[i].cpu().numpy()
        
        axes[i, 1].imshow(struct[0], cmap='gray', vmin=-1, vmax=1)
        axes[i, 1].set_title('Semantic')
        axes[i, 1].axis('off')
        
        axes[i, 2].imshow(struct[1], cmap='RdBu', vmin=-1, vmax=1)
        axes[i, 2].set_title('H-Distance')
        axes[i, 2].axis('off')
        
        axes[i, 3].imshow(struct[2], cmap='RdBu', vmin=-1, vmax=1)
        axes[i, 3].set_title('V-Distance')
        axes[i, 3].axis('off')
        
        # Overlay
        overlay = img.copy()
        mask = (struct[0] > 0).astype(np.float32)
        overlay[:, :, 1] = overlay[:, :, 1] * 0.5 + mask * 0.5
...
def train(args):
    ...
    print(f"Structures: {args.structures_dir}")
    
    dataset = ImageStructureDataset(args.images_dir, args.structures_dir)
    ...
    # Initialize model
    print("\nInitializing model...")
    model = ConditionalHistopathologyDiffusion(
        in_channels=3,
    ...
    print(f"  Dropout rate: {args.drop_prob} (for classifier-free guidance)")
    if use_amp:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if use_amp:
                with autocast(device_type='cuda', dtype=torch.float16):
                    loss = train_step_conditional(
                        model, diffusion, images, structures, 
                        optimizer, drop_prob=args.drop_prob
                    )
            else:
                loss = train_step_conditional(
                    model, diffusion, images, structures, 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train conditional histopathology image synthesis model')
    
    # Data arguments
    parser.add_argument('--images_dir', type=str, required=True,
                        help='Directory containing image .png files')
    parser.add_argument('--structures_dir', type=str, required=True,
                        help='Directory containing structure .npy files')
    parser.add_argument('--output_dir', type=str, default='outputs/image_synthesis',
    ...
    parser.add_argument('--drop_prob', type=float, default=0.1,
                        help='Dropout probability for classifier-free guidance (paper uses 0.1)')
    ...
    parser.add_argument('--guidance_scale', type=float, default=2.0,
                        help='Guidance scale for classifier-free guidance (paper uses w=2.0)')
